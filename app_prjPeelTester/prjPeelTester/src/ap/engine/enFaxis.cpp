...
bool enFaxis::IsInpose()
{
  bool ret = false;
  ret = abs((m_SendPos - m_AxisState.act_pos)) < 10;
  return ret;
}


int enFaxis::RelMove(int cmd_dist, uint32_t cmd_vel)
{
  bool ret;
  if (!IsMotorOn())
    return -1;
...
int enFaxis::Move (int cmd_pos, uint32_t cmd_vel, uint32_t acc, uint32_t decel)
{
  ...
  int conv_pos_pulse = 0;
  uint32_t conv_vel_pps = 0;
  ap_dat::dat_t cfg = m_pApCfgDat->apcfg_dat[static_cast<uint8_t>(ap_dat::addr_e::ap_mot_scale)];
  int turn_per_pulse = (int)cfg.parm1;
  int turn_per_move_mm = (int)cfg.parm2;
  int turn_per_move_um = turn_per_move_mm * 1000;
  //conv_pos = (((cmd_pos)/(int)turn_per_move_mm)*(int)turn_per_pulse)/1000;
  conv_pos_pulse = ((float)cmd_pos/(float)turn_per_move_um)*turn_per_pulse;
  conv_vel_pps = (uint32_t)(((float)cmd_vel/(float)60)*(float)turn_per_pulse);

  uint8_t data[9] ={0};
  data[0] = DEF_FASTECH_COMM_TYPE_MOVE_ABS_SINGLE_AXIS;
  data[1] = (uint8_t)(conv_pos_pulse>>0);
  data[2] = (uint8_t)(conv_pos_pulse>>8);
  data[3] = (uint8_t)(conv_pos_pulse>>16);
  data[4] = (uint8_t)(conv_pos_pulse>>24);
  data[5] = (uint8_t)(conv_vel_pps>>0);
  data[6] = (uint8_t)(conv_vel_pps>>8);
  data[7] = (uint8_t)(conv_vel_pps>>16);
  data[8] = (uint8_t)(conv_vel_pps>>24);
  ret = cmdFastech_SendCmd(&this->m_Packet,&data[0],9);
  uint32_t pass_ms = millis();
  uint8_t retry_cnt = 0;
...
int enFaxis::JogMove(uint32_t cmd_vel,bool is_cw)
{
  ...
  data[1] = (uint8_t)(conv_vel_pps>>0);
  data[2] = (uint8_t)(conv_vel_pps>>8);
  data[3] = (uint8_t)(conv_vel_pps>>16);
  data[4] = (uint8_t)(conv_vel_pps>>24);
  data[5] = (uint8_t)(is_cw);
  ret = cmdFastech_SendCmd(&this->m_Packet,&data[0],6);
...
int enFaxis::JogMove(uint32_t jog_pos, uint32_t cmd_vel, bool is_cw)
{
  ...
  uint32_t conv_pos_pulse = 0;
  uint32_t conv_vel_pps = 0;
  ap_dat::dat_t cfg = m_pApCfgDat->apcfg_dat[static_cast<uint8_t>(ap_dat::addr_e::ap_mot_scale)];
  uint32_t turn_per_pulse = (uint32_t)cfg.parm1;
  uint32_t turn_per_move_mm = (uint32_t)cfg.parm2;
  uint32_t turn_per_move_um = turn_per_move_mm * 1000;
  //conv_pos = (((cmd_pos)/(int)turn_per_move_mm)*(int)turn_per_pulse)/1000;
  conv_pos_pulse = (uint32_t)((float)jog_pos/(float)turn_per_move_um)*turn_per_pulse;
  conv_vel_pps = (uint32_t)(((float)cmd_vel/(float)60)*(float)turn_per_pulse);
...
int enFaxis::ClearState()
{
  bool ret;
  uint8_t data[1] ={0};
  data[0] = DEF_FASTECH_COMM_TYPE_ALARM_RESET;
  ret = cmdFastech_SendCmd(&this->m_Packet,&data[0],1);
...
int enFaxis::update_AxisState()
{
  ...
  m_pApReg->status[AP_REG_BANK_ERR_H][AP_REG_ERR_MOTOR_ERR_STATE] = m_AxisState.axis_status[DEF_FM_BANK_ERR_01][FM_REG_ERR_ALL];
  return ret;
}
