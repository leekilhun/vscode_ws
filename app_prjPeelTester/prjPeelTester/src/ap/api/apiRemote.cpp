...
#define UI_REMOTECTRL_CMD_CONTROL_CYL             0x18
#define UI_REMOTECTRL_CMD_CONTROL_VAC             0x19
#define UI_REMOTECTRL_CMD_CONTROL_MOT_RELMOVE     0x1A
#define UI_REMOTECTRL_CMD_CONTROL_MOT_CLEAR_ALARM 0x1B
...
#define UI_REMOTECTRL_WAIT_DELAY                           101
#define UI_REMOTECTRL_WAIT_RECEIVE                         102
#define UI_REMOTECTRL_MOTOR_MOVE_TIMEOUT                   1000*2


#define UI_REMOTECTRL_CYL_ACT_OPEN                         0x00
#define UI_REMOTECTRL_CYL_ACT_CLOSE                        0x01
#define UI_REMOTECTRL_CYL_ACT_GRIP                         0x02


#define UI_REMOTECTRL_BUFF_ASSIGN_DATA_ID                  0x00
#define UI_REMOTECTRL_BUFF_ASSIGN_DATA_IDX                 0x21   //id(1)+pose(8)*4 = 33
#define UI_REMOTECTRL_BUFF_ASSIGN_RETURN_ADDR              UI_REMOTECTRL_MAX_BUFFER_LENGTH-3
#define UI_REMOTECTRL_BUFF_ASSIGN_DELAY_MS                 UI_REMOTECTRL_MAX_BUFFER_LENGTH-2
#define UI_REMOTECTRL_BUFF_ASSIGN_RETURN_FLAG              UI_REMOTECTRL_MAX_BUFFER_LENGTH-1
...
void uiRemoteCtrl::ProcessCmd()
{
  ...
  case UI_REMOTECTRL_CMD_CONTROL_MOT_RELMOVE:
  {
    ...
    m_pAp->RelMove(cmd_dist, cmd_vel);
  }
    break;
...
void uiRemoteCtrl::doRunStep()
{
  ...
  /*######################################################
     wait delay and come back step
    ######################################################*/
  case UI_REMOTECTRL_WAIT_DELAY:
  {
    if (millis() - m_pre_time < (uint32_t)m_stepBuffer[UI_REMOTECTRL_BUFF_ASSIGN_DELAY_MS])
    {
      break;
    }
    m_stepBuffer[UI_REMOTECTRL_BUFF_ASSIGN_RETURN_FLAG] = 0x01;
    m_step.SetStep(m_stepBuffer[UI_REMOTECTRL_BUFF_ASSIGN_RETURN_ADDR]);
    m_pre_time = millis();
  }
  break;

  /*######################################################
     wait delay - to check receive
    ######################################################*/
  case UI_REMOTECTRL_WAIT_RECEIVE:
  {
  ...
  case UI_REMOTECTRL_STEP_DATA_SEND_ALL_DATA:
  {
    ...
    /*write data id*/
    m_stepBuffer[UI_REMOTECTRL_BUFF_ASSIGN_DATA_ID] = UI_REMOTECTRL_STEP_DATA_SEND_ALL_DATA;
    m_pre_time = millis();
  }
  break;
  case UI_REMOTECTRL_STEP_DATA_SEND_ALL_DATA_START:
  {
     ...
     */
    m_IsRespCplt = false;
    switch (m_stepBuffer[UI_REMOTECTRL_BUFF_ASSIGN_DATA_ID])
    {
    ...
    case UI_REMOTECTRL_STEP_DATA_SEND_ALL_DATA:
    default:
      m_stepBuffer[UI_REMOTECTRL_BUFF_ASSIGN_RETURN_ADDR] = UI_REMOTECTRL_STEP_DATA_SEND_ALL_DATA_WAIT;
  ...
  {
    /*  */
    m_stepBuffer[UI_REMOTECTRL_BUFF_ASSIGN_DATA_ID] = UI_REMOTECTRL_STEP_DATA_SEND_MOTOR_POS;
    axis_dat::dat_t data;
    uint32_t length = sizeof(axis_dat::dat_t);
    uint32_t index = 0;
    if (m_stepBuffer[UI_REMOTECTRL_BUFF_ASSIGN_RETURN_FLAG] == 0x00)
    {
      m_stepBuffer[UI_REMOTECTRL_BUFF_ASSIGN_RETURN_FLAG]= 0x01;
      //A flag to distinguish the data at the receiving point.
      m_stepBuffer[UI_REMOTECTRL_BUFF_ASSIGN_DATA_IDX] = 0x00;
    ...
    {
      m_stepBuffer[UI_REMOTECTRL_BUFF_ASSIGN_RETURN_FLAG]= 0x00;
      m_stepBuffer[UI_REMOTECTRL_BUFF_ASSIGN_DATA_IDX] = 0x01;
    ...
    cmdRobotro_SendCmd(&m_Packet, UI_REMOTECTRL_CMD_READ_MCU_DATA, &m_stepBuffer[0], (2 + (index * length)));

    m_IsRespCplt = false;

    if (m_stepBuffer[UI_REMOTECTRL_BUFF_ASSIGN_RETURN_FLAG] == 0x01)
    {
      m_stepBuffer[UI_REMOTECTRL_BUFF_ASSIGN_RETURN_FLAG]= 0x00;
      m_step.SetStep(UI_REMOTECTRL_WAIT_RECEIVE);
    }
    else
    {
      m_stepBuffer[UI_REMOTECTRL_BUFF_ASSIGN_RETURN_FLAG] = 0x01;
      ...
      if (m_stepBuffer[UI_REMOTECTRL_BUFF_ASSIGN_RETURN_ADDR])
      {
        m_step.SetStep(m_stepBuffer[UI_REMOTECTRL_BUFF_ASSIGN_RETURN_ADDR]);
      }
    }
