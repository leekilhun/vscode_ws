 ...
 *
 [ bank 0] error state [H]
  - bit 0          bit 1        bit 2       bit 3       bit 4       bit 5       bit 6       bit 7
    no error     no-resp LCD  no-resp Mot                                     motor_err  motor not ready
...
#define AP_REG_ERR_MOTOR_ERR_STATE   REG_BIT(6)
...
#define AP_REG_ERR_B12               REG_BIT(2)
#define AP_REG_ERR_B13               REG_BIT(3)
...
#define AP_REG_USE_DRY_RUN           REG_BIT(1)
#define AP_REG_SKIP_AREA_SENSOR      REG_BIT(2)
#define AP_REG_B23                   REG_BIT(3)
#define AP_REG_B24                   REG_BIT(4)
#define AP_REG_B25                   REG_BIT(5)
#define AP_REG_B26                   REG_BIT(6)
#define AP_REG_ALL_CEHCK_OK          REG_BIT(7)
...
#define AP_ERR_CODE(x)          static_cast<uint8_t>(Ap_reg::err_t::x)
  ...
  inline bool IsProcIniCplt() {
      return status[AP_REG_BANK_RUN_STATE][AP_REG_ORG_COMPLETED];
    }

  inline bool IsSkipAreaSensor() {
    return status[AP_REG_BANK_SETTING][AP_REG_SKIP_AREA_SENSOR];
  }

  inline bool IsAllCheckOk() {
     return status[AP_REG_BANK_SETTING][AP_REG_ALL_CEHCK_OK];
   }

  inline bool IsUseDryRun() {
    return status[AP_REG_BANK_SETTING][AP_REG_USE_DRY_RUN];
  }

  inline void SetUseDryRun(bool on_off){
    status[AP_REG_BANK_SETTING][AP_REG_USE_DRY_RUN] = on_off;
  }
  ...
  inline void SetRunState(bit_e bit, bool on_off = true){
    AP_REG_LOCK_BEGIN;
