...
#define FASTECH_CRC_POLY 0xA001
...
bool cmdFastech_SendCmdRxResp(fastech_t *p_cmd, uint8_t *p_data, uint8_t length, uint32_t timeout)
{
  bool  ret = false;
  uint32_t time_pre = 0;

  cmdFastech_SendCmd(p_cmd,p_data,length);

  time_pre = millis();
  while (1)
  {
    if (cmdFastech_ReceivePacket(p_cmd))
    {
      ret = true;
      break;
    }
